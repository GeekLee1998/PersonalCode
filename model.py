...
class DCGAN(object):
  ...
  def train(self, config):
    ...
    if config.dataset == 'mnist' or config.dataset == 'emnistdigits' or config.dataset == 'fashionmnist' or config.dataset == 'kmnist' or config.dataset == 'cifar10':
  ...
  def load_cifar10(self):
    #dirname = os.path.join(self.data_dir, self.dataset_name)

    X_train = []
    Y_train = []

    dirname = './data/cifar10/cifar-10-batches-py'

    for i in range(1, 6):
      self.fpath = os.path.join(dirname, 'data_batch_' + str(i))
      data, labels = self.load_batch(self.fpath)
      if i == 1:
        X_train = data
        Y_train = labels
      else:
    ...
    X_test, Y_test = self.load_batch(self.fpath)
